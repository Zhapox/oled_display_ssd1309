...
/**
 * SSD1309 OLED Driver for I2C bus (v1.7.13 – synchronous I2C)
 *
 * Changes from v1.7.1:
 *   - flush() address-reset command uses a pre-allocated Buffer
 *     (eliminates one Buffer.alloc per frame).
 *
 * Changes from v1.5.1:
 *   - fillRect() and hLine() write directly to the page buffer instead
 *     of calling setPixel() per pixel.  ~3-4× faster for large fills.
 *   - flush() reuses a pre-allocated chunk buffer instead of allocating
 *     8 new Buffers per frame.  Eliminates ~16 small GC allocs/sec.
 *   - Added rotate_180 option: flips SEG_REMAP and COM_SCAN in the init
 *     sequence for upside-down mounted displays.
 */
