...
/**
 * Renderer for the 128x64 OLED display (v1.7.13).
 *
 * Changes from v1.7.1:
 *   - Screensaver bounce uses off-axis velocity (1.2, 0.8) for varied
 *     path coverage.  Clock reposition interval is now time-based.
 *   - Progress bar borders use fillRect instead of individual setPixel.
 *   - Removed dead _calcVolumeWidth method.
 *   - Removed redundant new Date() in _renderIdle.
 *   - Added centering coupling comment on volume overlay.
 */
...
// Short day/month names for date display
...
function Renderer(display, options) {
  ...
  // Internal colon blink state (avoids drift from setTimeout vs system clock)
  this._colonVisible = true;
  this._colonLastToggle = Date.now();
  ...
  // Off-axis velocity (1.2, 0.8) produces a more varied path than 45°
  // diagonal (1, 1), covering more of the display for better burn-in
  // protection.
 ...
 * DVD-logo style, moves every frame.
 */
...
/**
 * Reset screensaver state (called when exiting screensaver).
 */
...
Renderer.prototype._renderIdle = function (state) {
  ...
  // Colon blink: toggle on/off every 1000ms using internal timer.
  // This avoids the drift caused by setTimeout not being synchronised
  // to the system clock (getSeconds() % 2 occasionally double-fires).
  var showColon = true;
  if (this.colonBlink) {
    var elapsed = Date.now() - this._colonLastToggle;
    if (elapsed >= 1000) {
      this._colonVisible = !this._colonVisible;
      this._colonLastToggle += 1000;
      // If we've drifted by more than one full cycle, resync
      if (Date.now() - this._colonLastToggle >= 1000) {
        this._colonLastToggle = Date.now();
      }
    }
  ...
  // ── Date, centred ─────────────────────────────────────────────────────
  var dateStr = DAY_NAMES[now.getDay()] + ' ' +
                String(now.getDate()) + ' ' +
